...
namespace dscd {
	...
	ObjectResponse<Message> BaseDiscordClient::sendMention(Snowflake<Channel> channelID, User user, std::string message, bool tts) {
		...
		return request(Post, path("channels/{channel.id}/messages", { channelID }),
			"{\"content\":\"<@!" + user.ID.string() + "> " + message + (tts ? "\",\"tts\":\"true\"" : "\"") + ", \"mentions\":" + user_str + "}");
	...
	ObjectResponse<Channel> BaseDiscordClient::createTextChannel(Snowflake<Server> serverID, std::string name) {
		return this->createChannel(serverID, name, Channel::SERVER_TEXT);
	}

	ObjectResponse<Channel> BaseDiscordClient::createTextChannel(Snowflake<Server> serverID, std::string name, Snowflake<Channel> parent_id) {
		return this->createChannel(serverID, name, Channel::SERVER_TEXT, parent_id);
	}

	ObjectResponse<Channel> BaseDiscordClient::createCategory(Snowflake<Server> serverID, std::string name) {
		return this->createChannel(serverID, name, Channel::SERVER_CATEGORY);
	}

	ObjectResponse<Channel> BaseDiscordClient::createChannel(Snowflake<Server> serverID, std::string name, Channel::ChannelType type) {
		return request(Post, path("guilds/{guild.id}/channels", { serverID }), "{\"name\": \"" + name + "\", \"type\": " + std::to_string(static_cast<int>(type)) + "}");
	}

	ObjectResponse<Channel> BaseDiscordClient::createChannel(Snowflake<Server> serverID, std::string name, Channel::ChannelType type, Snowflake<Channel> parent_id) {
		return request(Post, path("guilds/{guild.id}/channels", { serverID }), "{\"name\": \"" + name + "\", \"type\": " + std::to_string(static_cast<int>(type)) + ", \"parent_id\": " + parent_id.string() + "}");
	}
