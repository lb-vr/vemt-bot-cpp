...
//#define TOKEN_SIZE 64
	...
	public:
		...
		ObjectResponse<Message     > sendMessage             (Snowflake<Channel> channelID, std::string message, bool tts = false);                                          //to do add more then one message return
		ObjectResponse<Message     > sendMention             (Snowflake<Channel> channelID, User user, std::string message, bool tts = false);
		...
		ObjectResponse<Channel     > createTextChannel       (Snowflake<Server> serverID, std::string name, Snowflake<Channel> parent_id);
		ObjectResponse<Channel     > createCategory          (Snowflake<Server> serverID, std::string name);
		ObjectResponse<Channel     > createChannel	         (Snowflake<Server> serverID, std::string name, Channel::ChannelType type);
		ObjectResponse<Channel     > createChannel	         (Snowflake<Server> serverID, std::string name, Channel::ChannelType type, Snowflake<Channel> parent_id);
